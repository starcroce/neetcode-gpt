...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        for _ in range(num_blocks):
            tf_block = self.TransformerBlock(model_dim, num_heads)
            self.tf_blocks.append(tf_block)

        self.final_ln = nn.LayerNorm(model_dim)
        self.final_proj = nn.Linear(model_dim, vocab_size)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        token_embs = self.vocab_emb(context)
        batch_size, seq_len = context.shape
        pos_embs = self.pos_emb(torch.arange(seq_len))
        embs = token_embs + pos_embs

        outputs = self.final_ln(self.tf_blocks(embs))
        logits = self.final_proj(outputs)
