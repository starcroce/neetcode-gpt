...
class Solution:
    def create_batches(self, data: TensorType[int], context_length: int, batch_size: int) -> Tuple[TensorType[int], TensorType[int]]:
        ...
        torch.manual_seed(0)
        start_pos_list = torch.randint(len(data) - context_length, (batch_size,))
        
        x = [data[i : i + context_length] for i in start_pos_list]
        x = torch.stack(x)

        y = [data[i + 1 : i + 1 + context_length] for i in start_pos_list]
        y = torch.stack(y)
